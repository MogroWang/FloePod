...
当前源码版本 **1.6.0**，提供 x64 Windows 便携包、NSIS、MSI 和 MSIX 构建路径。
...
- **1.6.0**（当前）：修复浮动面板四周多出的白色边框（停止对面板清除框架样式位，保留 tao 的系统阴影架构，仅压制 Win11 1px 外描边与焦点过渡）；修复边缘浮动条偶发白色方角标题栏（消除显示 / 焦点 / 拖动路径上对 WebView2 合成的反复打断，显示后强制重新呈现）
