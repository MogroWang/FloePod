...
当前源码版本 **1.6.1**，提供 x64 Windows 便携包、NSIS、MSI 和 MSIX 构建路径。
...
- **1.6.1**（当前）：保持兼容的设置、面板、Windows 窗口与文件事务重构；补强隐私检查资源限制、后台任务退出与版本发布门禁。
- **1.6.0**：修复浮动面板四周多出的白色边框（停止对面板清除框架样式位，保留 tao 的系统阴影架构，仅压制 Win11 1px 外描边与焦点过渡）；修复边缘浮动条偶发白色方角标题栏（消除显示 / 焦点 / 拖动路径上对 WebView2 合成的反复打断，显示后强制重新呈现）
- **1.5.0**：辅助功能各选项独立生效（移除总开关模式）；高级设置中规则匣与敏感匣拆分独立卡片；浮动条新增「长度」与「填充色」设置并统一命名；浮动面板宽度范围调整为 410–600px；修复边缘浮动条偶发矩形标题栏与材质切换后不立即刷新；优化多选勾选按钮与右键菜单交互
...
- PR 阶段完成质量门禁，合并后自动构建与发布，流程及 required checks 配置见 [自动发布说明](docs/release.md)
- 兼容性边界和本轮验证见 [工程说明](docs/engineering.md)；真实桌面环境检查见 [Windows 检查单](docs/release-checklist.md)，不作为自动 Release 的人工等待步骤
