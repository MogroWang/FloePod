...
当前源码版本 **1.7.2**，提供 x64 Windows 便携包、NSIS、MSI 和 MSIX 构建路径。
...
- **1.7.2**（当前）：修复剪切模式把文件拖回自身仍可能删除文件的问题（落点记录与源路径统一「原始 + 规范化」双键匹配，清理判定写入日志）；修复浮动面板顶部的空缺边框线（窗口层恢复被 tao 顶部内缩占用的客户区，WebView 铺满窗口）
