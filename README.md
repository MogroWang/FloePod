...
当前源码版本 **1.7.3**，提供 x64 Windows 便携包、NSIS、MSI 和 MSIX 构建路径。

## 功能

- **多匣体系**：多个「匣」各自独立贴在屏幕的上/下/左/右边缘，可分别设置所在显示器、沿边缘位置、暂存文件夹、不透明度与材质（亚克力 / 纯半透明），随时启用或停用；浮动条图标可自定义（emoji），暂存数量徽标可显可隐
...
- **暂存面板**：悬停或单击浮匣弹出（不抢焦点）；文件缩略图、多选（点选 / 框选）、列表与图标平铺两种视图、批量「复制到… / 移动到…」导出、拖出（复制 / 剪切）、移出暂存（进回收站，可反悔）
...
- **1.7.3**（当前）：剪切拖回自身新增与落点记录独立的终保底（源文件仍在暂存目录内时无条件保留，根治 1.7.2 残余丢失）；面板顶部恢复改为无条件执行并写入日志（修复 1.7.2 防护被瞬时窗口矩形误拦导致的时隐时现）；复制拖回自身改为创建后缀副本而非忽略
- **1.7.2**：修复剪切模式把文件拖回自身仍可能删除文件的问题（落点记录与源路径统一「原始 + 规范化」双键匹配，清理判定写入日志）；修复浮动面板顶部的空缺边框线（窗口层恢复被 tao 顶部内缩占用的客户区，WebView 铺满窗口）
- **1.7.1**：规则匣与敏感匣设置重组为单匣卡片双分组，匣切换按钮常显、开关控件统一、文案精简；修复浮动面板顶部在 Windows 11 上出现的白色/无色细线（tao 客户区顶部内缩留下的非客户区被 DWM 默认 caption 色填充，现在请求不绘制 caption）；修复匣的浮动条 emoji 图标与暂存数量开关保存失败（字段未接入后端补丁白名单）
- **1.7.0**：新增匣级浮动条 emoji 图标与暂存数量徽标显隐设置；浮动面板新增多选模式（点选 / 框选）与图标平铺视图，视图一键切换；右键菜单收起时淡出；移除面板头部的选择文件 / 文件夹入口（拖入与 `Alt+数字` 投递不受影响），暂存文字正文随面板高度伸展；修复剪切模式下把文件拖回同一个匣导致文件被错误删除的严重问题（自拖完全屏蔽，跨匣移动不受影响）
- **1.6.1**：保持兼容的设置、面板、Windows 窗口与文件事务重构；补强隐私检查资源限制、后台任务退出与版本发布门禁。
