...
当前源码版本 **1.7.0**，提供 x64 Windows 便携包、NSIS、MSI 和 MSIX 构建路径。
...
- **1.7.0**（当前）：新增匣级浮动条 emoji 图标与暂存数量徽标显隐设置；浮动面板新增多选模式（点选 / 框选）与图标平铺视图，视图一键切换；右键菜单收起时淡出；移除面板头部的选择文件 / 文件夹入口（拖入与 `Alt+数字` 投递不受影响），暂存文字正文随面板高度伸展；修复剪切模式下把文件拖回同一个匣导致文件被错误删除的严重问题（自拖完全屏蔽，跨匣移动不受影响）
