...
当前源码版本 **1.7.1**，提供 x64 Windows 便携包、NSIS、MSI 和 MSIX 构建路径。
...
- **1.7.1**（当前）：规则匣与敏感匣设置重组为单匣卡片双分组，匣切换按钮常显、开关控件统一、文案精简；修复浮动面板顶部在 Windows 11 上出现的白色/无色细线（tao 客户区顶部内缩留下的非客户区被 DWM 默认 caption 色填充，现在请求不绘制 caption）；修复匣的浮动条 emoji 图标与暂存数量开关保存失败（字段未接入后端补丁白名单）
