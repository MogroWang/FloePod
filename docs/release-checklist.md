...
- [ ] 在空数据目录完成首次引导，再通过“新建匣”创建第二个匣；两者默认面板宽度均为 440px，用户选择的名称、位置和外观正确保留
- [ ] 四边浮动条在反复显示/隐藏、点击面板/右键菜单切换焦点、拖动位置及文件拖入加宽后不出现系统标题栏或白色矩形；同时确认面板阴影、键盘焦点和拖放正常
...
- [ ] 辅助功能在 125% / 150% / 200% 下无不可达控件，仅键盘可以完成选择文件、暂存、搜索、导出、撤销和关闭
