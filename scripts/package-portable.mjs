...
3. 面板里可以：
   - 点“文件”或“文件夹”按钮投递，不必拖拽；也可复制文件后按 Ctrl+V
   - 把条目拖出到任何程序 / 文件夹（底部可切换「复制 / 剪切」）
   - 多选后「复制到… / 移动到…」批量导出
   - 用“安全交接”在本机检查隐私元数据、生成清理副本或 SHA-256 交接包
   - 移出暂存（文件进回收站，可反悔）
4. 文字也能暂存：热键收集剪贴板（默认 Alt+Shift+S），或面板里点「文字」按钮
5. 设置里的“安心中心”提供操作时间线、24 小时撤销、本地搜索和大界面模式

【快捷键（默认，可在设置中修改）】
  Alt+Shift+F  显示 / 隐藏浮匣
  Alt+Shift+S  收集剪贴板文字到暂存
  Alt+Shift+P  打开暂存面板
  Alt+Shift+L  立即锁定所有敏感匣
  Alt+1..9     选择文件并投递到对应编号的匣
...
【卸载】删除本文件夹即可；若开启过「开机自启」或资源管理器
“发送到 FloePod”，请先在设置中关闭，以移除对应的当前用户注册表项。
